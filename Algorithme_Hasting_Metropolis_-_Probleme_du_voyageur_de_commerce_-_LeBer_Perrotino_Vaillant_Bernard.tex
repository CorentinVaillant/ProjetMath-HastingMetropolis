...
\begin{tcolorbox}[colback=white,colframe=green!80!black,title=Démonstration]
    ...
    Ainsi, un saut partant de $x$ est simulé en choisissant un candidat $y$ par la loi $Q(x, \cdot)$, puis on saute sur $y$ avec probabilité $\min\big(1, \frac{\pi(y) Q(y, x)}{\pi(x) Q(x, y)} \big)$, sinon on reste sur $x$.\\
    ...
    $$
    \mathbb{P}(X_{n+1} = y \mathbf{ et } U_{n+1} \leq TODO)
    $$
