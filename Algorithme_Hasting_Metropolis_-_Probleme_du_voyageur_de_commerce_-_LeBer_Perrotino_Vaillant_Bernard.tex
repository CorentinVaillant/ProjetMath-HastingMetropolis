...
\usepackage[a4paper, top=35mm, bottom=35mm, left=35mm, right=35mm]{geometry} % Modification des tailles des marges (on en parlera après).
...
Dans cette configuration, la probabilité de transition de l'état $1$ vers l'état $2$ est donnée par :
\[
...
\[
\mathbb{E}(N_j \mid X_0 = i) = \sum_{n \in \mathbb{N}^{*}} \cdot \mathbb{P}(X_n = j \mid X_0 = i) = \sum_{n \in \mathbb{N}^{*}} P_{i,j}^{(n)}.
\]
\end{tcolorbox}
