...
\noindent\textbf{Temps Discret et Temps Continu}
\begin{leftbar}
Soit $T$ un ensemble d'indices représentant le temps. \\

On distingue généralement deux types de modélisation temporelle :
\begin{enumerate}[leftmargin=5em, label=(\arabic*)]
  \item Un processus est dit \textbf{à temps discret} si $T = \mathbb{N}$ ou $T = \mathbb{Z}_+$. Dans ce cas, les évolutions du processus se produisent à des instants distincts et dénombrables.
  \item Un processus est dit \textbf{à temps continu} si $T = \mathbb{R}_+$. Cela signifie que le processus évolue en tout instant dans un continuum temporel.
\end{enumerate}
\end{leftbar}

En d'autres termes, un processus à temps discret est observé uniquement à des instants discrets (intercouppés pas des intervalles de temps fixes et généralement indexés par des nombres entiers), tandis qu'un processus à temps continu évolue de manière fluide dans le temps, sans interruption.

Cette distinction joue un rôle fondamental dans la classification et l'analyse des \textbf{processus stochastiques}. \\


\noindent\textbf{Processus Stochastique}
\begin{leftbar}
Soit $(\Omega, \mathcal{F}, \mathbb{P})$ un espace de probabilité.

Soit $T$ un ensemble d'indices (souvent $T = \mathbb{N}$, $\mathbb{Z}_+$ ou $\mathbb{R}_+$). \\

Un \textbf{processus stochastique} est une famille de variables aléatoires $\{X_t\}_{t \in T}$ définies sur $(\Omega, \mathcal{F}, \mathbb{P})$ et à valeurs dans un espace d'états $E$.

Autrement dit, pour tout $t \in T$, $X_t$ est une variable aléatoire prenant ses valeurs dans $E$, et l'évolution de $\{X_t\}_{t \in T}$ décrit une dynamique aléatoire au cours du temps.
\end{leftbar}
...
En termes mathématiques, une chaîne de Markov est un processus de Markov à temps discret ou à temps continu et à espace d'états discret. Un processus de Markov est un processus stochastique possédant la propriété de Markov : l'information utile pour la prédiction du futur est entièrement contenue dans l'état présent du processus et n'est pas dépendante des ses états antérieurs.
...
\noindent\textbf{Graphe Orienté d'une Chaîne de Markov}
\begin{leftbar}
Une chaîne de Markov peut être représentée sous forme d'un graphe orienté $ G = (V, A) $, où :
\begin{enumerate}[leftmargin=5em, label=(\arabic*)]
    \item $ V $ est l'ensemble des sommets, correspondant aux états de l'espace d'états $ E $.
    \item $ A $ est l'ensemble des arêtes orientées pondérées, où une arête de $ i $ vers $ j $ est présente si et seulement si $ P_{i,j} > 0 $, avec une pondération correspondant à la probabilité de transition $ P_{i,j} $.
\end{enumerate}
\end{leftbar}

Cette représentation graphique d'une chaîne de Markov permet d’illustrer visuellement les différentes dynamiques de transitions entre chaque états et de mieux appréhender la structure du processus stochastique. \\
