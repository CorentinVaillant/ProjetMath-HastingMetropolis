...
Il existe d'autres méthodes de Monte Carlo qui, selon les cas, peuvent s’avérer plus ou moins pertinentes que la méthode de Hastings-Metropolis. En voici une liste non exhaustive, accompagnée d’une brève explication pour chacune :

\begin{enumerate}
    \item \textbf{L'échantillonnage de Gibbs} : Comme la méthode de Hastings-Metropolis, cette approche consiste à construire une chaîne de Markov dont la distribution stationnaire est une distribution cible $\pi$. 
    L’échantillonnage de Gibbs met à jour chaque composante d'un vecteur aléatoire de manière séquentielle, conditionnellement aux autres. On génère ainsi aléatoirement $N$ vecteurs de dimension $n$ suivant la distribution $\pi$.
    Cette méthode est particulièrement efficace dans le cas où la distribution conditionnelles $\pi(x \mid \cdot )$ est facile à échantillonner.
