...
Par résoluble, il est important de préciser qu'il s'agit d'une résolution appoximative parce que le TSP est NP-difficile comme mentionné précédemment.
...
Précisons toutefois que HM n'est habituellement pas la méthode que l'on utilise pour le TSP en pratique. Usuellement, le nombre de ville étant grand (suffisemment pour que la solution par brute force ne soit pas trouvable) mais pas absurde (une centaine par exemple), on préfère utiliser des versions déterministes optimisées comme l'algorithme de Held-Karp de complexité $O(2^n \times n^2)$, ou dans le cas de situation avec encore plus de villes, des méthodes similaires à HM plus adaptées comme l'échantillonage de Gibbs.
...
\newpage % TODO garder les "newpage" vraiment util
...
% TODO garder les "newpage" vraiment util
...
On pose :
\[
\tau_i = \inf\{\exists n \in \mathbb{N}^{*}, n \geq 1 \mid X_n = i\}
\]
...
% TODO garder les "newpage" vraiment util
    ...
    \begin{itemize}
        ...
        \item Chaque état de la classe est alors transient car nous ne reviendrons pas aux états $i \in \{3, 4\}$ après un nombre infini de transitions.
    \end{itemize}
...
\subsection{Exemples d'applications simples de HM} %Nouveau titre plus clair, enlever ce commentaire après le merge
...
À noter que certains états peuvent être selectionnés même s'ils semblent se situer dans des zones de faible densité (c'est le cas ici de $x_3$). \\
...
La différence notable avec le précédent exemple est que l'on ne cherche plus à échantilloner des points sur une courbe de densité mais des chemins entre les villes. Chaque possibilité est donc un ordre de visite des villes, et il en existe $5! = 120$ au total. \\
