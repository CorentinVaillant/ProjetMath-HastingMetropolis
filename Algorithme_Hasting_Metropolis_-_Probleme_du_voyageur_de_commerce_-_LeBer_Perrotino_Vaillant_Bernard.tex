...
\begin{tcolorbox}[colback=white,colframe=green!80!black,title=Démonstration,breakable]
On fixe $n \in \mathbb{N}^{*}$. Nous allons maintenant montrer que $\pi P^n = \pi$. \\

Pour tout $i \in E$ :
\[
(\pi P^n)_i = \sum_{j \in E}\pi_j(P^n)_{j,i}
\]
Or, par la décomposition des puissances,
\[
(P^n)_{j,i} = \sum_{k \in E}(P^{n-1})_{j,k} P_{k,i}.
\]
Donc :
\[
(\pi P^n)_i = \sum_{j \in E}\pi_j \cdot \sum_{k \in E}(P^{n-1})_{j,k} P_{k,i} = \sum_{k \in E}\Bigl[\sum_{j \in E}\pi_j (P^{n-1})_{j,k}\Bigr]P_{k,i}.
\]
Or, par hypothèse d'invariance pour la puissance $n-1$, on a :
\[
\sum_{j \in E}\pi_j(P^{n-1})_{j,k} = (\pi P^{n-1})_k = \pi_k
\]
Ainsi :
\[
(\pi P^n)_i = \sum_{k \in E}\pi_kP_{k,i} = \pi_i.
\]
Ce qui montre bien que $\pi P^n = \pi$ pour tout $n \in \mathbb{N}^{*}$.
\end{tcolorbox}
...
\begin{itemize}
    \item Si $x = y$ : la propriété est vraie.
    \item Si $x \neq y$ :
        \begin{itemize}
            \item Si $Q(x, y) = Q(y, x) = 0$, alors :
            \[
...
\subsection{Brêves descriptions d'autres méthodes MCMC permettant de résoudre le TSP}
