...
\begin{tcolorbox}[colback=white,colframe=black!80!black,title=Ancienne version de la sous-section~\ref{subsubsec: Matrice de transition en k-pas} (Partie à supprimer)]
...
La matrice $P^k$ est obtenue en élevant la matrice de transition $P$ à la puissance $k$, ce qui revient à effectuer $k-1$ produits matriciels successifs :
\[
P^k = \underbrace{P \cdot P \cdot \ldots \cdot P}_\textit{k fois}.
\]
...
\begin{tcolorbox}[colback=white,colframe=black!80!black,title=Récurrence Positive et Convergence Vers la Distribution Stationnaire (Partie à supprimer)]
